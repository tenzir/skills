...
@timestamp
:   All events must populate [`@timestamp`](../fields/at_timestamp.yaml) with the event’s original timestamp.

message
:   Most events should populate [`message`](../fields/message.yaml).

ecs.version
:   The referenced [`ecs.version`](../fields/ecs_version.yaml) used to develop the data mapping or ingest pipeline. This value helps detect when mappings update or fall behind. It can also help explain why a particular data source isn’t populating the same fields as another.

tags and labels
:   The [`tags`](../fields/tags.yaml) and [`labels`](../fields/labels.yaml) fields add simple metadata as `keyword` values.
...
The [`host.*`](../fieldsets/host.yaml) field set contains common attributes for different computing instances. Certain host types have more fields to capture specific details, like `cloud.*` or `container.*`.

## Agent and observer [_agent_and_observer]

An agent is software that collects, observes, measures, or detects the event. The [`agent.*`](../fieldsets/agent.yaml) fields capture details about which agent entity captured the event, including the agent’s version. Examples of agents are Beats and Elastic Agent.

An `observer` is an external monitoring or intermediary device, like a firewall, APM server, or web proxy. These devices monitor and detect network, security, application events. Capture the details for these device types in the [`observer.*`](../fieldsets/observer.yaml) field set.
...
@timestamp
:   All events must populate [`@timestamp`](../fields/at_timestamp.yaml) with when the event originated.
...
event.start
:   This timestamp marks the beginning of the event activity. For example, in a network session, [`event.start`](../fields/event_start.yaml) is the timestamp of the first observed packet in the flow.

event.end
:   This timestamp marks the end of the activity. In a network flow, [`event.end`](../fields/event_end.yaml) is the timestamp of the last observed packet in the flow.
...
event.module
:   If the ingest agent or pipeline has a concept of modules or plugins, populate [`event.module`](../fields/event_module.yaml) with the module or plugin name.

event.dataset
:   Used to define different types of logs or metrics from an event source. The recommended convention is `<moduleName>.<datasetName>`. For Apache web server access logs, the [`event.dataset`](../fields/event_dataset.yaml) value will be `apache.access`.
...
[Using the Categorization Fields](categorization-usage.md) covers more details on using these four fields together to categorize events.
...
Many events have similar content populating different fields: IP addresses, file hashes, hostnames. Pivot between these events using the [`related.*`](../fieldsets/related.yaml) fields.

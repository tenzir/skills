...
When an event contains details about the sending and receiving hosts, the baseline for capturing these values will be the [source](../fieldsets/source.yaml) and [destination](../fieldsets/destination.yaml) fields.

Some events may also indicate each host’s role in the exchange: client or server. When this information is available, the [client](../fieldsets/client.yaml) and [server](../fieldsets/server.yaml) fields should be used *in addition to* the `source` and `destination` fields. The fields and values mapped under `source`/`destination` should be copied under `client`/`server`.
...
Note this event contains additional details that would populate additional fields (such as the [DNS Fields](../fieldsets/dns.yaml)) if this was a complete mapping example. These additional fields are omitted here to focus on the network details.
...
When considering the [event categorization fields](https://github.com/elastic/ecs/blob/401807e0547301525acd28c4fb667203fec66d59/docs/reference/ecs-category-field-values-reference.md), the `category` and `type` fields are populated using their respective allowed values which best classify the source network event.

...
canonical: https://tenzir.com/docs/reference/operators/context_lookup
source: https://tenzir.com/docs/reference/operators/context_lookup.md
section: "Docs"
---
...
```tql
context_lookup name:string, key=field|list, [into=field, yield=field,
...
```tql
context_lookup "feodo", key=src_ip
...
```tql
context_lookup "feodo", key=src_ip, live=true
where @name == "suricata.flow"
```

### Apply context to historical events

Apply the context `feodo` to historical `suricata.flow` events with every update to `feodo`. In this example, on any `context_update` to the `fedo` context, historical data persisted in the node will be retro-matched against the update.

```tql
context_lookup "feodo", key=src_ip, retro=true
where @name == "suricata.flow"
```

...
2. **Format**. If your *X* is a wire format, either text-based like JSON or binary like PCAP, NetFlow, or IPFIX, then look for [parsing operators](../reference/operators.md#parsing) and [printing operators](../reference/operators.md#printing). Those start with `read_*` and `write_*`, respectively. For example, [`read_pcap`](https://tenzir.com/docs/reference/operators/read_pcap.md) parses PCAP streams, while [`read_netflow`](https://tenzir.com/docs/reference/operators/read_netflow.md) parses raw NetFlow and IPFIX messages. Similarly, there exist [parsing functions](../reference/functions.md#parsing) and [printing functions](../reference/functions.md#printing) that start with `parse_*` and `print_*`.

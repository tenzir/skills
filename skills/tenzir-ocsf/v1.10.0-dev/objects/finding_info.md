# Finding Information (finding_info)

The Finding Information object describes metadata for a security finding generated by a security tool, analytic, or system. The `analytic` identifies the rule, detection logic, model, or algorithm used to produce the finding. The `related_events` contain metadata about events, activities, or other findings associated with this finding, including inputs that contributed to the analytic result or results identified through `query_info`.

- **Extends**: [Object (object)](object.md)

## Attributes

### `analytic`

- **Type**: [`analytic`](analytic.md)
- **Requirement**: recommended

The analytic technique used to analyze and derive insights from the data or information that led to the finding or conclusion.

### `attack_graph`

- **Type**: [`graph`](graph.md)
- **Requirement**: optional
- **Group**: context

An Attack Graph describes possible routes an attacker could take through an environment. It describes relationships between resources and their findings, such as malware detections, vulnerabilities, misconfigurations, and other security actions.

### `attacks`

- **Type**: [`attack`](attack.md)
- **Requirement**: optional

An array of attack objects, each describing a tactic, technique, and/or sub-technique associated with the finding.

### `created_time`

- **Type**: `timestamp_t`
- **Requirement**: recommended

The time when the finding was created.

### `data_sources`

- **Type**: `string_t`
- **Requirement**: optional

A list of data sources utilized in generation of the finding.

### `desc`

- **Type**: `string_t`
- **Requirement**: optional

The description of the reported finding.

### `first_seen_time`

- **Type**: `timestamp_t`
- **Requirement**: optional

The time when the finding was first observed. e.g. The time when a vulnerability was first observed.

It can differ from the `created_time` timestamp, which reflects the time this finding was created.

### `kill_chain`

- **Type**: [`kill_chain_phase`](kill_chain_phase.md)
- **Requirement**: optional

The Kill Chain provides a detailed description of each phase and its associated activities within the broader context of a cyber attack.

### `last_seen_time`

- **Type**: `timestamp_t`
- **Requirement**: optional

The time when the finding was most recently observed. e.g. The time when a vulnerability was most recently observed.

It can differ from the `modified_time` timestamp, which reflects the time this finding was last modified.

### `modified_time`

- **Type**: `timestamp_t`
- **Requirement**: optional

The time when the finding was last modified.

### `product`

- **Type**: [`product`](product.md)
- **Requirement**: optional

Details about the product that reported the finding.

### `product_uid`

- **Type**: `string_t`
- **Requirement**: optional

> **Deprecated since v1.4.0.** Use the `product.uid` attribute instead.

The unique identifier of the product that reported the finding.

### `query_info`

- **Type**: [`query_info`](query_info.md)
- **Requirement**: optional

Query information for related_events. Populate `query_info.query_string` with the search criteria used to identify events, activities, or observables related to the finding.

Note:The query does not need to be limited to events that directly contributed to the finding's `analytic` result. It may also describe a broader relevant result set, such as related observables, follow-up activity, or other events that can be investigated or tracked after the finding is created.

### `related_analytics`

- **Type**: [`analytic`](analytic.md)
- **Requirement**: optional

Other analytics related to this finding.

### `related_events`

- **Type**: [`related_event`](related_event.md)
- **Requirement**: optional

Describes events and/or other findings related to the finding as identified by the security product. Note that these events may or may not be in OCSF.

### `related_events_count`

- **Type**: `integer_t`
- **Requirement**: optional

Number of related events or findings.

### `resolved_time`

- **Type**: `timestamp_t`
- **Requirement**: recommended
- **Group**: occurrence

The time when the finding was resolved. Populate when the finding's `status_id` is 4 (Resolved).

### `src_url`

- **Type**: `url_t`
- **Requirement**: optional

The URL pointing to the source of the finding.

### `tags`

- **Type**: [`key_value_object`](key_value_object.md)
- **Requirement**: optional

The list of tags; `{key:value}` pairs associated with the finding.

### `title`

- **Type**: `string_t`
- **Requirement**: recommended

A title or a brief phrase summarizing the reported finding.

### `traits`

- **Type**: [`trait`](trait.md)
- **Requirement**: optional

The list of key traits or characteristics extracted from the finding.

### `types`

- **Type**: `string_t`
- **Requirement**: optional

One or more types of the reported finding.

### `uid`

- **Type**: `string_t`
- **Requirement**: required

The unique identifier of the reported finding.

### `uid_alt`

- **Type**: `string_t`
- **Requirement**: optional

The alternative unique identifier of the reported finding.

...
Tenzir nodes include a light-weight storage engine on top of Feather or Parquet files, accessible via the [`import`](/reference/operators/import.md) and [`export`](/reference/operators/export.md) operators.

The storage engine comes with a catalog that tracks schema meta data and a thin layer of indexing to accelerate queries.

Our [tuning guide](../guides/node-setup/tune-performance.md) has further details on the inner workings.

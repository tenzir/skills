...
You have two ways to pass an argument to an operator that expects a secret. The following examples use the [`to_splunk`](http://docs.tenzir.com/reference/operators/to_splunk.md) operator, which expects a HEC-token for authentication:
...
* Use the [`secret`](http://docs.tenzir.com/reference/functions/secret.md) function ([Managed Secret](#managed-secrets)):
...
Tenzir’s [type system](../reference/types.md) includes secrets as a special type. You can access secrets only with the [`secret`](http://docs.tenzir.com/reference/functions/secret.md) function.
...
Since secrets can be values in a pipeline, you can pass them through the [`python`](http://docs.tenzir.com/reference/operators/python.md) operator. You must not modify a secret in the `python` operator.
...
Access a managed secret’s value with the [`secret`](http://docs.tenzir.com/reference/functions/secret.md) function.
...
Tenzir hides the `tenzir.secrets` section from the [`config`](http://docs.tenzir.com/reference/functions/config.md) function.

...
`unflatten` uses a heuristic to determine the unflattened schema. Thus, the schema of a record that has been flattened using [`flatten`](http://docs.tenzir.com/reference/functions/flatten.md) and unflattened afterwards may not be identical to the schema of the unmodified record.

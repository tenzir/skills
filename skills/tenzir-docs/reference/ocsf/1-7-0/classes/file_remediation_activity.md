...
* **Requirement**: required
* **Values**:
  * `7002` - `File Remediation Activity`: File Remediation Activity events report on attempts at remediating files. It follows the MITRE countermeasures defined by the D3FEND™ [Matrix](https://d3fend.mitre.org/). Sub-techniques will include File, such as File Removal or Restore File.
...
* **Values**:
  ...
  * `700201` - `File Remediation Activity: Isolate`: Creates logical or physical barriers in a system which reduces opportunities for adversaries to create further accesses. Defined by D3FEND™ [d3f:Isolate](https://d3fend.mitre.org/tactic/d3f:Isolate/).
  * `700202` - `File Remediation Activity: Evict`: Removes an adversary or malicious resource from a device or computer network. Defined by D3FEND™ [d3f:Evict](https://d3fend.mitre.org/tactic/d3f:Evict/).
  * `700203` - `File Remediation Activity: Restore`: Returns the system to a better state. Defined by D3FEND™ [d3f:Restore](https://d3fend.mitre.org/tactic/d3f:Restore/).
  * `700204` - `File Remediation Activity: Harden`: Increases the opportunity cost of computer network exploitation. Defined by D3FEND™ [d3f:Harden](https://d3fend.mitre.org/tactic/d3f:Harden/).
  * `700205` - `File Remediation Activity: Detect`: Further identify adversary access to or unauthorized activity on computer networks. Defined by D3FEND™ [d3f:Detect](https://d3fend.mitre.org/tactic/d3f:Detect/).
...
The event class name, as defined by class\_uid value: `File Remediation Activity`.

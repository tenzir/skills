...
> OCSF article: Profiles are Powerful
...
Finally, what if a class or object wants to be considered as part of the profile family but wants to add new attributes that are only relevant to the one particular class or object? This may sound a bit esoteric, but it has already been used in the resource\_details object for the Cloud profile. When the Cloud profile is applied to classes with attributes of the resource\_details object type, for example, API Activity, the cloud\_partition and region attributes defined within the object are added, but only when the Cloud profile is applied to the class. The event now includes the api and cloud attributes, while the resource\_details object of the class adds the other two attributes - effectively creating a custom hybrid profile.

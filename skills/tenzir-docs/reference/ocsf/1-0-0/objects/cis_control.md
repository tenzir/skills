# CIS Control

> The CIS Control contains information as defined by the Center for Internet Security Critical Security Control [(CIS CSC)](https://www.cisecurity.org/controls).


The CIS Control contains information as defined by the Center for Internet Security Critical Security Control [(CIS CSC)](https://www.cisecurity.org/controls). Prioritized set of actions to protect your organization and data from cyber-attack vectors.

## Attributes

**`control`**

* **Type**: `string_t`
* **Requirement**: required

The CIS critical security control.

**`version`**

* **Type**: `string_t`
* **Requirement**: recommended

The CIS critical security control version.

## Used By

* [`security_finding`](../classes/security_finding.md)
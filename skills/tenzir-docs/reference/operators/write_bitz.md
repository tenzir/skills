...
Internally, BITZ uses Arrow’s IPC format for serialization and deserialization, but prefixes each message with a 64 bit size prefix to support changing schemas between batches—something that Arrow’s IPC format does not support on its own. Use [`write_bitz`](/reference/operators/write_bitz.md) when you want to persist events in a compact columnar format and later load them again with [`read_bitz`](/reference/operators/read_bitz.md).

...
The subpipeline receives events and must return bytes. For example, use [`write_ndjson`](/reference/operators/write_ndjson.md) to serialize events as newline-delimited JSON.

## Examples

### Upload events as NDJSON

Use [`write_ndjson`](/reference/operators/write_ndjson.md) to serialize each event as one JSON object per line before uploading it.
...
Add [`compress_gzip`](/reference/operators/compress_gzip.md) to the printing subpipeline when you want to upload compressed output.
...
* [`from_ftp`](/reference/operators/from_ftp.md)

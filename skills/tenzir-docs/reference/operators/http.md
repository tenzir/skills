...
http url:string, [method=string, body=record|string|blob, encode=string,
     ...
     error_field=field, paginate=record->string|string, paginate_delay=duration,
...
Use `paginate="link"` to follow RFC 8288 `Link` headers automatically:
...
This is useful for APIs that include pagination URLs in the `Link` response header with `rel=next`, such as GitHub, GitLab, and Jira.

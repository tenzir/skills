...
The format to write inferred from the file-ending. Supported file formats are the common file endings for our [`read_*` operators](../operators.md#parsing).
...
The compression, just as the format, is inferred from the “file-ending” in the URI. Under the hood, this uses the [`decompress_*` operators](../operators.md#encode--decode). Supported compressions can be found in the [list of compression extensions](#compression).
...
| `smtp`, `smtps`, `mailto`, `email` | [`save_email`](save_email.md)                           | `to "smtp://john@example.com"`                 |

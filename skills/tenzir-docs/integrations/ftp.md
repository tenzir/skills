...
Tenzir supports the [File Transfer Protocol (FTP)](https://en.wikipedia.org/wiki/File_Transfer_Protocol), both downloading and uploading files. Use [`from_ftp`](/reference/operators/from_ftp.md) to download bytes and parse them with a subpipeline, and use [`to_ftp`](/reference/operators/to_ftp.md) to print events with a subpipeline and upload the result.
...
Use [`from_ftp`](/reference/operators/from_ftp.md) with [`read_ndjson`](/reference/operators/read_ndjson.md) to turn the downloaded bytes into events.
...
Use [`to_ftp`](/reference/operators/to_ftp.md) with [`write_ndjson`](/reference/operators/write_ndjson.md) to serialize events before uploading them.

...
* [`read_syslog`](http://docs.tenzir.com/reference/operators/read_syslog.md): a parser that turns unstructured Syslog messages into structured events.
* [`write_syslog`](http://docs.tenzir.com/reference/operators/write_syslog.md): a printer that transforms structured events into compliant Syslog messages.
...
To receive Syslog messages on a UDP socket, use [`accept_udp`](http://docs.tenzir.com/reference/operators/accept_udp.md):
...
To use TCP instead of UDP, use [`accept_tcp`](http://docs.tenzir.com/reference/operators/accept_tcp.md) with [`read_syslog`](http://docs.tenzir.com/reference/operators/read_syslog.md):
...
The pipeline inside [`accept_tcp`](http://docs.tenzir.com/reference/operators/accept_tcp.md) executes *for each accepted connection*.
...
When you throw [`read_syslog`](http://docs.tenzir.com/reference/operators/read_syslog.md) at this line, you’ll get this output:
...
Note that the `content` field is just a big string. Parse it with [`parse_cef`](http://docs.tenzir.com/reference/functions/parse_cef.md):
...
Tenzir auto-detects octet-counted messages in both [`read_syslog`](http://docs.tenzir.com/reference/operators/read_syslog.md) for streaming input and [`parse_syslog`](http://docs.tenzir.com/reference/functions/parse_syslog.md) for parsing individual strings. Use the `octet_counting` parameter to require or disable this behavior.
...
Tenzir also supports **creating** Syslog messages from structured events via [`write_syslog`](http://docs.tenzir.com/reference/operators/write_syslog.md).
...
The [`write_syslog`](http://docs.tenzir.com/reference/operators/write_syslog.md) operator converts the `structured_data` field into a valid [RFC 5424](https://datatracker.ietf.org/doc/html/rfc5424) structured block.
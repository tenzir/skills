...
- `security_result.about` / `securityResult.about`: Bad URLs, IPs, and files embedded within the email body.
...
- Only populate the `email` field in `security_result.about` / `securityResult.about` or `network.email`.
...
- `security_result` / `securityResult`: Describe the malicious activity detected.
...
- `network`: Include details of the network connection. You must populate the following fields: `network.ip_protocol` / `network.ipProtocol` `network.application_protocol` / `network.applicationProtocol`
...
- `security_result` / `securityResult`: Add one or more items to the `security_result` / `securityResult` field to represent the malicious activity detected.
...
- `target`: `target.registry`: If the target registry is remote, target must include at least one identifier for the target machine (for example, an IP address, MAC, hostname, or third party asset identifier). `target.registry.registry_key` / `target.registry.registryKey`: All registry events must include the affected registry key.

#### Optional Fields

- `security_result` / `securityResult`: Describe the malicious activity detected. For example, a bad registry key.
- `principal.user`: Populate if user information is available about the process.
...
- `intermediary`: For SSO logins, intermediary must include at least one machine identifier for the SSO server if available.
- `network` and `network.http`: If the login occurs over HTTP, you must place all available details in `network.ip_protocol` / `network.ipProtocol`, `network.application_protocol` / `network.applicationProtocol`, and `network.http`.
...
- `security_result` / `securityResult`: Add a `security_result` / `securityResult` field to represent the login status if it fails. Specify `security_result.category` / `securityResult.category` with the AUTH_VIOLATION value if authentication fails.
